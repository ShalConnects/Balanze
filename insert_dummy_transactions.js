...
const userEmail = 'bebetteryou.motivational@gmail.com';

async function insertDummyTransactions() {
  console.log('🚀 Starting to insert dummy transactions for', userEmail);
  console.log('User ID:', userId);
  
  try {
    // First, let's find the user's cash account
    console.log('\n1. Finding user\'s cash account...');
    ...
    if (!accounts || accounts.length === 0) {
      console.log('❌ No cash account found for user. Creating one...');
      ...
      console.log('✅ Created cash account:', newAccount.id);
      var cashAccountId = newAccount.id;
    } else {
      console.log('✅ Found cash account:', accounts[0].id, '(' + accounts[0].name + ')');
      var cashAccountId = accounts[0].id;
    }
