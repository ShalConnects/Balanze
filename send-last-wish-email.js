/**
 * Standalone Last Wish Email Sender
 * 
 * This script can be run directly to send Last Wish emails
 * Usage: node send-last-wish-email.js <userId>
 */
...
async function sendLastWishEmail(userId) {
  try {
...
async function main() {
  ...
  if (!userId) {
    console.log('Usage: node send-last-wish-email.js <userId>');
    console.log('Example: node send-last-wish-email.js 123e4567-e89b-12d3-a456-426614174000');
    process.exit(1);
  }

  const result = await sendLastWishEmail(userId);
