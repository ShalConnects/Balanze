-- Fix lend/borrow triggers to work with existing transactions table structure
-- This removes the currency field from transaction inserts since it doesn't exist
...
-- Step 4: Create new functions WITHOUT currency field
...
BEGIN
    ...
    -- Create transaction based on type (without currency field)
...
BEGIN
    ...
    IF NEW.status = 'settled' AND OLD.status != 'settled' AND NEW.transaction_id IS NOT NULL THEN
        ...
        IF account_record IS NOT NULL THEN
            -- Create repayment transaction (opposite of original) - without currency field
            IF NEW.type = 'lend' THEN
...
BEGIN
    ...
    IF NEW.partial_return_amount > 0 AND (OLD.partial_return_amount IS NULL OR OLD.partial_return_amount != NEW.partial_return_amount) THEN
        ...
        IF account_record IS NOT NULL THEN
            -- Create partial return transaction (opposite of original) - without currency field
            IF NEW.type = 'lend' THEN
...
SELECT 'Fix completed successfully - triggers now work with existing transactions table' as status;
