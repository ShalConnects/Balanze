-- Convert Specific Records to Record-Only Mode
-- This will remove account linkage but keep transaction_id for reference

-- Step 1: Update the specific records to record-only mode
UPDATE lend_borrow 
SET 
    account_id = NULL,  -- Remove account linkage
    affect_account_balance = false  -- Set to record-only mode
WHERE id IN (
...
SELECT 
    ...
    'Records converted to record-only' as description,
    COUNT(*) as count
FROM lend_borrow 
WHERE account_id IS NULL 
AND affect_account_balance = false;

-- Step 3: Show sample of converted records
SELECT 
    id,
    type,
    person_name,
...
-- Step 4: Show settled vs active breakdown
SELECT 
    'SETTLED_RECORDS' as section,
    'Settled record-only records' as description,
    COUNT(*) as count
FROM lend_borrow 
WHERE account_id IS NULL 
AND affect_account_balance = false
...
SELECT 
    'ACTIVE_RECORDS' as section,
    'Active record-only records' as description,
    COUNT(*) as count
FROM lend_borrow 
WHERE account_id IS NULL 
AND affect_account_balance = false
