// Try to import Fuse.js, fallback to simple search if not available
let Fuse: any;
try {
  Fuse = require('fuse.js');
} catch (error) {
  console.warn('Fuse.js not available, using fallback search implementation');
  Fuse = null;
}
...
export function rankSearchResults<T>(
  ...
  }).sort((a, b) => (a.finalScore || 0) - (b.finalScore || 0)); // Lower score = better match in Fuse.js
}
...
export class UnifiedSearchService {
  private fuseInstances: Map<string, any> = new Map();
  ...
  /**
   * Simple fuzzy search fallback when Fuse.js is not available
   */
  ...
  search<T>(
    ...
    try {
      // Try to use Fuse.js
      const fuse = this.getFuseInstance(data, dataType, config);
      results = fuse.search(expandedQuery) as SearchResult<T>[];
    } catch (error) {
      // Fallback to simple search
      console.warn('Fuse.js not available, using fallback search');
      results = this.simpleFuzzySearch(data, expandedQuery, config.keys, config.threshold || 0.3);
    }
    ...
    // Store event ID for click tracking
    if (limitedResults.length > 0) {
      limitedResults.forEach((result, index) => {
        (result as any).__searchEventId = eventId;
        (result as any).__searchRank = index;
      });
    }

    // Track zero results
    if (limitedResults.length === 0) {
      trackZeroResults(query, this.getCurrentPage(), searchTime);
    }
  ...
  trackResultClick(result: any, itemType: string, itemId: string, itemTitle: string): void {
    if (result.__searchEventId) {
      trackSearchClick(result.__searchEventId, itemType, itemId, itemTitle);
    }
  }
  ...
  trackSuggestionUsage(originalQuery: string, suggestion: string): void {
    trackSuggestionClick(originalQuery, suggestion, this.getCurrentPage());
  }
