...
export const AccountsView: React.FC = () => {
  ...
  const handlePrint = (startDate?: string, endDate?: string) => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Account Statement - ${escapeHtml(selectedAccount.name)}</title>
          <style>
            @media print {
              @page { margin: 1cm; }
              body { margin: 0; }
        ...
        <body>
          <div class="header">
            ...
            <div class="account-info">
              <p><strong>Account:</strong> ${escapeHtml(selectedAccount.name)}</p>
              <p><strong>Type:</strong> ${escapeHtml(selectedAccount.type.charAt(0).toUpperCase() + selectedAccount.type.slice(1))}</p>
              <p><strong>Currency:</strong> ${escapeHtml(selectedAccount.currency)}</p>
              <p><strong>Period:</strong> ${escapeHtml(periodText)}</p>
              <p><strong>Generated:</strong> ${escapeHtml(format(new Date(), 'MMM dd, yyyy h:mm a'))}</p>
            </div>
          </div>
          
          <div class="summary">
            <h2>Summary</h2>
            <p><strong>Total Income:</strong> ${escapeHtml(formatCurrency(income, selectedAccount.currency))}</p>
            <p><strong>Total Expenses:</strong> ${escapeHtml(formatCurrency(expenses, selectedAccount.currency))}</p>
            <p><strong>Net Amount:</strong> ${escapeHtml(formatCurrency(net, selectedAccount.currency))}</p>
            <p><strong>Current Balance:</strong> ${escapeHtml(formatCurrency(selectedAccount.calculated_balance || 0, selectedAccount.currency))}</p>
          </div>
          
          <h2>Transactions</h2>
          ${groupedTransactions.map(([groupLabel, groupTransactions]) => `
            <div class="date-group">
              <div class="date-group-header">${escapeHtml(groupLabel)}</div>
              <table>
                <thead>
                  <tr>
                    <th>Date</th>
                ...
                <tbody>
                  ${groupTransactions.map(t => `
                    <tr>
                      ...
                      <td>${escapeHtml(formatCurrency(t.amount, selectedAccount.currency))}</td>
                    </tr>
                  `).join('')}
                </tbody>
              </table>
          ...
          <div class="footer">
            <p>Generated by Balanze on ${escapeHtml(format(new Date(), 'MMM dd, yyyy h:mm a'))}</p>
          </div>
        </body>
      </html>
    `);
    ...
    try {
      if (moveToMainAccount) {
        ...
        if (!cashAccount) {
          ...
          await fetchAccounts();
          const created = useFinanceStore.getState().accounts.find(
            (a) => a.type === 'cash' && a.currency === dpsCurrency && a.name === newAccountName
          );
