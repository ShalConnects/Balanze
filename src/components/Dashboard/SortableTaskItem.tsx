...
const SortableTaskItemComponent: React.FC<SortableTaskItemProps> = ({
  ...
  return (
    ...
    >
      <div className="flex items-start gap-1 sm:gap-1.5 p-1 sm:p-1.5">
        {/* Drag Handle - Only this area is draggable */}
        <div
          {...attributes}
          {...listeners}
          className="flex-shrink-0 cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors pt-0.5 min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center"
        ...
        >
          <div className="flex items-center justify-between gap-1.5 mb-0.5">
            <div className="font-medium text-[10px] sm:text-xs md:text-sm text-gray-900 dark:text-white truncate flex-1 min-w-0">
              {task.title}
            </div>
          ...
          <div className="flex items-center justify-between gap-1.5 mb-0.5">
            <div className="text-[9px] sm:text-[10px] md:text-xs text-gray-500 dark:text-gray-400 truncate flex-1 min-w-0">
            {clientName}
          </div>
          {task.due_date && (
              <div className="text-[9px] sm:text-[10px] md:text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap flex-shrink-0">
                {isTouchDevice 
          ...
          {task.description && (
            <button
              ...
              className="mt-0.5 text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 flex items-center gap-0.5 transition-colors min-h-[40px] sm:min-h-0 py-0.5 sm:py-0 touch-manipulation"
              type="button"
      ...
      {showActions && (
        ...
        >
          <button
            ...
            className="p-1.5 sm:p-0.5 text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors rounded min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center"
            title="Change status"
          >
          ...
          <button
            ...
            className="p-1.5 sm:p-0.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors rounded min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center"
            title="Edit task"
          >
          ...
          <button
            ...
            className="p-1.5 sm:p-0.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors rounded min-w-[40px] min-h-[40px] sm:min-w-0 sm:min-h-0 flex items-center justify-center"
            title="Delete task"
          >
      ...
      {statusMenuOpen === task.id && (
        <div 
          ref={statusMenuRef}
          className={`${isTouchDevice ? 'absolute' : 'fixed'} z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg py-0.5 px-0.5 flex items-center gap-0.5 flex-wrap ${
            isTouchDevice 
              ? menuPositionAbove
                ? 'bottom-full left-0 right-0 mb-1 max-w-full'
                : 'top-full left-0 right-0 mt-1 max-w-full'
              : ''
          }`}
          style={
            isTouchDevice 
        ...
        >
          {[
            ...
            <button
              ...
              className={`px-1.5 py-1.5 sm:px-1 sm:py-0.5 text-[9px] sm:text-[10px] md:text-xs rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors whitespace-nowrap min-h-[40px] sm:min-h-0 ${
